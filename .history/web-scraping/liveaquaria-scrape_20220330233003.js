...
(async () => {
    ...
    const fishCNames = await page.$$(".category-list> .category-item > '.cat-name categorypage-ev-tracking'");
    ...
    for(let li of fishCNames)
    {
        ...
        commonNameArr[k] = sNameText2.trim();
        console.log(commonNameArr[j]);
        k++;
    }
    let j = 0;
    // //Get fish names, put them in array set
    for(let li of fishSNames)
    {
        ...
        sNameText1 = sNameString.split('(');
        sNameText2 = sNameString.split(')');
        sciNameArr[j] = sNameText2.trim();
        console.log(sciNameArr[j]);
        j++;
    }

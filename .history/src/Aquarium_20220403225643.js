...
const Aquarium = () => {
  ...
  let [tankSize, setTankSize] = useState(0);
 ...
 if (testTankSize(inputLength, inputWidth, inputHeight) == true)
 {
  ...
  if(testTemperature(fishTank, fish) == true)
  {
    if(testPH(fishTank, fish) == true)
    {
      ...
      setTankCapacity(1-((fishTank.Size - fish.averageSize*1.5)/fishTank.Size))*100);
  ...
  const progressInstance = <ProgressBar now={tankCapacity} label={`${tankCapacity}%`} />;

...
const Aquarium = () => {
  ...
  let [fishTank, setFishTank] = useState(
    new Tank(inputLength, inputWidth, inputHeight, 0, 0, 0, 0, 0, 0)
  );
  ...
  const addFish = (value) => {
    ...
    if (testTankSize(inputLength, inputWidth, inputHeight) == true) {
      timeout = setTimeout(value.fishID, 1).toString();
      const d = new Date();
      let ms = d.getMilliseconds();

      id = timeout.toString() + "_" + ms;
  ...
  const removeFish = (value) => {
    ...
    if (userList.length <= 0) {
      sessionStorage.setItem(
        "tank",
        JSON.stringify(new Tank(0, 0, 0, 0, 0, 0, 0, 0, 0))
      );

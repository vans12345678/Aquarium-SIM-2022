...
const Compendium = () => {
  ...
  const getFish = () => {
    Axios.get("https://mysql-aquarium.herokuapp.com:3001/fishGet").then((response) => {
  ...
  const searchFishAll = () => {
    Axios.post("https://mysql-aquarium.herokuapp.com:3001/fishComp", { search: search }).then(
      (response) => {
        setFishList(response.data);
      }
    );
  ...
  useEffect(() => {
    console.log(process.env.REMOTE_DATABASE);
    getFish();
  }, []);

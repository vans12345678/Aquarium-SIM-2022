...
(async () => {
    ...
    for(let i = 0; i < nameArrSet.length; i ++)
    {
        ...
        {
            //Common name
            commonNameArr[i] = nameArrSet[i].substr(0, nameArrSet[i].indexOf('('));
            //commonNameArr.push(nameArrSet[i]);
        }   
    }
    for(let i = 0; i < commonNameArr.length; i++)
    {
        console.log(commonNameArr[i]);
    }

    console.log(priceArr.length + " " + commonNameArr.length);

...
const Aquarium = () => {
  ...
  let [fishTank, setFishTank] = useState(
    new Tank(inputLength, inputWidth, inputHeight, 0, 0, 0, 0)
  );
  ...
  const addFish = (value) => {
    ...
    if (testTankSize(inputLength, inputWidth, inputHeight) == true) {
      ...
      if (testTemperature(fishTank, userList, fish) == true) {
        ...
        function image() {
          let img = $('<img className="aquariumFish">');
          img.attr("src", fish.image);
          img.className = "aquariumFish";
        }
        $(image()).insertAfter(aquariumImg);
  ...
  const removeFish = (value) => {
    ...
    if (userList.length <= 0) {
      sessionStorage.setItem(
        "tank",
        JSON.stringify(new Tank(0, 0, 0, 0, 0, 0, 0))
      );

...
const Aquarium = () => {
  ...
  const renderFish = () => {
    ...
    arrFish.forEach((fish) => {
      ...
      var animationDuration = 10000; // in milliseconds
        ...
        if (testTemperature(fishTank, fish) === true) {
          if (testPH(fishTank, fish) === true) {
            if (testFishSize(userList, fish, fishTank) === true) {
              ...
              //get the aquarium image element
              let aquariumImg = document.getElementById("aquarium");
              //create a new image element with the source of the fish image and the unique fishKey id
              var $img = $("<img />", {
                src: require("./images/" + fish.image),
                id: fish.fishKey,
              });
              ...
              //insert the fish image element after the aquarium image
              $($img).insertAfter(aquariumImg);

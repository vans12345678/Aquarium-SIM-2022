...
const Listings = () => {
  ...
  const addFish = () =>{
    Axios.post("http://localhost:3001/fishAdd", {search: search}).then((response) => {
      setFishList(response.data);
    });
    console.log(search);
  }
        ...
        <div className="table">
          ...
          return (
              <tr key={val.fishMatchID}>
                ...
                <td><a tabindex="0" class="btn btn-lg btn-danger" role="button" data-toggle="popover" data-trigger="focus" title="Dismissible popover" data-content="And here's some amazing content. It's very engaging. Right?">Dismissible popover</a></td>
                <td><button onClick={addFish} type="button" class="btn btn-primary">Add</button></td>
              </tr>
          );
        })}
          </tbody>

...
const Aquarium = () => {
  ...
  const setTankDimensions = () =>
  {
    ...
    if(testTankSize(inputLength, inputWidth, inputHeight) == true)
    {
      ...
      fishTank.size = Math.round(parseInt(inputLength)*parseInt(inputWidth)*parseInt(inputHeight)/1000);
 ...
 if (testTankSize(inputLength, inputWidth, inputHeight) == true)
 {
  ...
  const removeFish = (value) => {
    ...
    userList.forEach(element => {
      ...
      //   setFishTank(fishTank);
      // }
      console.log(fishTank.length);
      console.log(element);
      console.log(fishTank.capacity);
      fishTank.capacity = (fishTank.capacity+(1-((fishTank.size - (element))/fishTank.size))*100);
      setFishTank(fishTank);
      sessionStorage.setItem("tank", JSON.stringify(fishTank));

...
export const testFishSize = (userList, newFish, tank) => {
  ...
  let tank.lowerFishSize = userList[0].averageSize;
  let tank.upperFishSize = userList[0].averageSize;

  //Check to see if the new fish being added has a valid size
  if(newFish.averageSize < tank.lowerFishSize || newFish.averageSize > tank.upperFishSize)
  {
  ...
  userList.forEach(element => {
    ...
    if(element.averageSize < tank.lowerFishSize)
    {
      tank.lowerFishSize = element.averageSize;
    }
    //Tet for upper range
    if(element.averageSize > tank.upperFishSize)
    {
      tank.upperFishSize = element.averageSize;
    }
  });

...
const Listings = () => {
  ...
  const addFish = () =>{
    Axios.post("http://localhost:3001/addFish", {search: search}).then((response) => {
      setFishList(response.data);
    });
    //console.log(search);
  }
        ...
        <div className="table">
          ...
          return (
              <tr key={val.fishMatchID}>
                ...
                <td><button onClick={(event) => { setAddList(event.target.value); }} type="button" className="btn btn-primary">Add</button></td>
              </tr>
          );
        })}
          </tbody>

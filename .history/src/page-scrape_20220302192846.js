...
(async () => {
    ...
    for (let i = 0; i < 1; i++)
    {
        let fishScientificName = fullFishSpecArrays[i][0];
        let fishCommonName = fullFishSpecArrays[i][1]; 
        let fishAverageSize = fullFishSpecArrays[i][2]; 
        let fishLowerPH = fullFishSpecArrays[i][3]; 
        let fishUpperPH = fullFishSpecArrays[i][4]; 
        let fishLowerTemp = fullFishSpecArrays[i][5];
        let fishUpperTemp = fullFishSpecArrays[i][6]; 
        let fishAggrSameSpecies = fullFishSpecArrays[i][7]; 
        let fishAggrOtherSpecies =fullFishSpecArrays[i][8];
        let fishLocationTank=fullFishSpecArrays[i][9];
        
        const sqlInsert = "INSERT INTO tblfish(fishScientificName, fishCommonName, fishAverageSize, fishLowerPH, fishUpperPH, fishLowerTemp, fishUpperTemp, fishAggrSameSpecies, fishAggrOtherSpecies, fishLocationTank)" + 
        " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",[fishScientificName, fishCommonName, fishAverageSize, fishLowerPH, fishUpperPH, fishLowerTemp, fishUpperTemp, fishAggrSameSpecies, fishAggrOtherSpecies,fishLocationTank]; 

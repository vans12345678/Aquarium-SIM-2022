...
import setPageNumbe from 'react-paginate';
...
const Compendium = () => {
  ...
  //function for retrieving all fish entries
    const getFish = () => {
      if (fishList.length === 0 && currentPage === 1){
  ...
  const currentEntry = fishList.slice(indexOfFirstEntry, indexOfLastEntry);
  const pageCount = Math.ceil(fishList.length / entriesPerPage);

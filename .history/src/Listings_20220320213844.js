...
const Listings = () => {
  ...
  const searchFish = () =>{
    Axios.post("http://localhost:3001/listings").then((response) => {
      setFishList(response.data);
    });
  }
  ...
  return (
    <div>
      <section className="home">
        ...
        <input type="text" id="search" name="search" onChange={getInputValue}></input>
        <br />
        <br />
      ...
      <section>
        ...
        <div className="table">
          ...
          return (
              <tr>
                <td> {val.tetraCommonName}</td>
                <td>{val.tetraScientificName}</td>
                <td>${val.tetraPrice}</td>
                <td><a href={val.tetraURL}>{val.tetraStoreName}</a></td>
              </tr>
          );
        })}
          </tbody>
